...
def process_csv_file(bucket_name, source_key):
    ...
    connection = connect_to_database() if not header_error else None
    ...
    try:
        for row in reader:
            ...
            insert_error = insert_sale(connection, parsed_row, clean_row)
            if insert_error:
                invalid_rows.append(add_error(clean_row, insert_error))
    finally:
        if connection:
            connection.close()
...
def insert_sale(connection, parsed_row, clean_row):
    # Insert only valid records; id and fecha_registro are generated by Postgres.
    ...
    try:
        ...
        connection.rollback()
        return f"database insert failed: {error}"
    finally:
        cursor.close()
